...
> Answer: One of the benefit of using `client-side routing` is that full page reload is no longer required. With traditional server-side rendering, the server responds by generating and returning a completely new page for every request/interaction. It slows down the load time, uses more bandwidth, and creates a less responsive user experience(UX). Client-side rendering avoids making unnecessary requests for a full page when only a portion/component of the page has changed. 
